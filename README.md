...
| Prompts | Unique human prompt events recognized by each adapter. Text is reduced immediately to word and character counts; supported attachment-only events may count as prompts but not as prompt-style samples. Hermes counts one stored prompt event per session, exact timestamp and content, so compaction copies of one message count once while simultaneously live rows stay separate. |
| Projects | Distinct final directory names associated with top-level sessions. Absolute project paths are not persisted. |
| Tool calls | Provider-scoped native counts: deduplicated call identifiers where exposed, otherwise a mapped source count. There is no cross-provider sum or ranking, and a native counter is not necessarily monotonic: Hermes reports its stored active-transcript count, which compaction, rewind or transcript replacement can lower. An unsupported count, such as Cursor's, is shown as unavailable rather than measured zero. |
...
The aggregate uses schema **3**, which removes `totals.tool_calls`. Consumers should read `providers[].tool_calls` together with `tool_calls_available` and preserve each provider's native semantics. Each artifact also records `generator_version`, the executable that produced it (`dev` marks a source build; an empty value marks an artifact written before this field existed).
