...
The writer creates and syncs a temporary file beside the destination, then renames it over the prior artifact. On Unix-like systems, the product directory is set to mode `0700` and the temporary/final artifact to mode `0600`. On Windows, the product directory is created or updated with a protected, inheritable DACL limited to the current user and LocalSystem and validated before the artifact is created. Synthetic Windows amd64/arm64 CI verifies this directory protection and artifact creation; real Windows harness stores remain unverified.
