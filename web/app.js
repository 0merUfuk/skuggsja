...
(function () {
  ...
  // loads an external asset or a brand file: every path below is baked-in
  // data, sourced once and vendored, never fetched at run time.
  const solidGlyphs = { claude: true, codex: true, cursor: true };
  // Harness marks. Claude, Codex and Cursor carry their published monochrome
  // marks as path data (Claude/Codex/Cursor: simple-icons, CC0-1.0, each
  // icon's declared source is its vendor's own brand page). Hermes Agent is
  // an in-house orchestrator with no public brand to source from, so its mark
  // reproduces the supplied design mockup's geometry instead (mockup-derived,
  // not an official mark — see CHANGELOG.md).
  ...
  // Cursor: simple-icons "Cursor" mark (CC0-1.0), sourced from Cursor's own
  // published brand page; full attribution recorded in CHANGELOG.md.
  const CURSOR_MARK = "M11.503.131 1.891 5.678a.84.84 0 0 0-.42.726v11.188c0 .3.162.575.42.724l9.609 5.55a1 1 0 0 0 .998 0l9.61-5.55a.84.84 0 0 0 .42-.724V6.404a.84.84 0 0 0-.42-.726L12.497.131a1.01 1.01 0 0 0-.996 0M2.657 6.338h18.55c.263 0 .43.287.297.515L12.23 22.918c-.062.107-.229.064-.229-.06V12.335a.59.59 0 0 0-.295-.51l-9.11-5.257c-.109-.063-.064-.23.061-.23";
  const glyphShapes = {
    claude: [["path", { d: CLAUDE_MARK }]],
    codex: [["path", { d: CODEX_MARK }]],
    cursor: [["path", { d: CURSOR_MARK }]],
    // Nested hollow triangles carrying a heavier stroke than the default
    // outline weight, with a small notch at the bottom centre: this is the
    // supplied mockup's geometry, reproduced faithfully in the absence of a
    // public Hermes Agent mark.
    hermes: [
      ["path", { d: "M12 3.3 21.4 20.5h-6.9L12 22.6l-2.5-2.1H2.6Z", "stroke-width": "2.1" }],
      ["path", { d: "M12 11.7 16.3 19h-8.6Z", "stroke-width": "2.1" }]
    ],
    unknown: [
...
(function () {
  ...
  function renderProjectsByTool(rawProviders) {
    ...
    const providers = arrayOrEmpty(rawProviders).filter(isRecord).map(function (provider) {
      return {
        id: cleanText(provider.id, "unknown", 80),
    ...
    providers.forEach(function (provider) {
      ...
      // The row names a harness, so its meter carries that harness's series
      // colour exactly as the usage key and the model index do.
      row.setAttribute("data-harness", provider.id);
      row.append(
