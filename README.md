...
| --- | --- | --- | --- |
| Claude Code | `projects` below `CLAUDE_CONFIG_DIR`, otherwise `~/.claude/projects` | Root sessions, human prompts, models, tool calls, source-recorded message token ledger | Real data on macOS |
| Codex | `sessions` and `archived_sessions` below `CODEX_HOME`, otherwise `~/.codex`; rollout `.jsonl` and `.jsonl.zst` files | Root/child sessions, human prompts, models, tool calls, final cumulative source-recorded token fields | Real data on macOS; compressed-file handling is fixture-tested |
...
Missing histories are reported as `not found`; they do not make the run fail. Parser and schema problems appear as provider-scoped warnings. See [Architecture](ARCHITECTURE.md) for the adapter contract and [Privacy](PRIVACY.md) for the data lifecycle.
...
| `--no-source-audit` | Skip before/after source hashing. This does not change the read-only parser design, but the run cannot claim source integrity was verified. |
...
| `CLAUDE_CONFIG_DIR` | Base Claude configuration directory; `projects` is appended unless the specific Skuggsja override is set |
| `CODEX_HOME` | Base Codex directory; `sessions` and `archived_sessions` are appended unless their specific overrides are set |
...
| Metric | Definition |
| --- | --- |
| Sessions | Top-level histories with a usable start time. Child/subagent histories are excluded. |
...
| Longest session | Largest non-negative `end - start` duration among top-level sessions. |
| Late night | Percentage of top-level sessions whose activity hour is 00:00 through 04:59 in the machine's local timezone. |
...
Coverage runs from the earliest top-level session start to the latest top-level session end found locally. `calendar_framing` becomes true only when a same-year span reaches the first and last seven days of that year; it is a presentation hint, not a completeness claim. Deleted, cloud-only, moved, or unsupported histories cannot be proven absent.
...
- Claude nested subagent transcript files are excluded because copied fork history can duplicate events.
